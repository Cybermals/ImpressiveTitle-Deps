...
void GrassLayer::setMaterialName(const String &matName)
{
	if (material.isNull() || matName != material->getName()){
		material = static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName(matName));
		if (material.isNull())
...
void GrassLayer::_updateShaders()
{
	if (shaderNeedsUpdate){
		...
		if (caps->hasCapability(RSC_VERTEX_PROGRAM) && geom->getShadersEnabled())
		{
			...
			MaterialPtr tmpMat = static_cast<MaterialPtr>(MaterialManager::getSingleton().getByName(matName));
			if (tmpMat.isNull())
			{
				...
				HighLevelGpuProgramPtr vertexShader = static_cast<HighLevelGpuProgramPtr>(HighLevelGpuProgramManager::getSingleton().getByName(vsName));
